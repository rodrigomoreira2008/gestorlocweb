...
const emptyForm: GrupoParceiro = { NOME: '', STATUS: 'ATIVO', CONTROLE: '' };

export function GrupoParceirosPage() {
  const [rows, setRows] = useState<GrupoParceiro[]>([]);
  const [q, setQ] = useState('');
  const [form, setForm] = useState<GrupoParceiro>(emptyForm);
  const [open, setOpen] = useState(false);

  async function load() {
    setRows(await api<GrupoParceiro[]>(`/grupo-parceiros?q=${encodeURIComponent(q)}`));
  }
  ...
  async function submit(event: FormEvent) {
    ...
    const path = form.ID ? `/grupo-parceiros/${form.ID}` : '/grupo-parceiros';
  ...
  async function remove(id?: number) {
    ...
    await api(`/grupo-parceiros/${id}`, { method: 'DELETE' });
    await load();
  }

  return (
    <>
      <PageHeader title="Grupo Parceiros" description="Cadastre e organize os grupos de parceiros.">
